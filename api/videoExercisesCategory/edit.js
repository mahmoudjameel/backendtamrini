...
router.post("/", async (req, res) => {
  try {
    ...
    let productSearch = await videoExercisesCategory.findOne({ _id: product._id });
    ...
    if (image) {
      ...
      const result = await videoExercisesCategory.updateOne(
        { _id: product._id },
        {
          name,
    ...
    /********************************************************/
    //Get the new product
    productSearch = await videoExercisesCategory.findOne({ _id: product._id });
    productSearch.image = `${req.protocol}://${req.headers.host}/images/video-exercise-categories/${productSearch.image}`
    ...
    console.log(`Error in /VideoExercisesCategory/edit, error: ${e.message}`, e);
    res.json({
      status: false,
      errors: [e.message],
    });
