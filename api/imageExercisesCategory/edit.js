...
router.post("/", async (req, res) => {
  try {
    ...
    let productSearch = await ImageExerciseCategoryModel.findOne({ _id: product._id });
    ...
    /********************************************************/
    //Get the new product
    productSearch = await ImageExerciseCategoryModel.findOne({ _id: product._id });
    productSearch.image = `${req.protocol}://${req.headers.host}/images/image-exercise-categories/${productSearch.image}`
    ...
    console.log(`Error in /ImageExercisesCategory/edit, error: ${e.message}`, e);
    res.json({
      status: false,
      errors: [e.message],
    });
