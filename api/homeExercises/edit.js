  ...
  try {
    ...
    switch (+exercise.type) {
      case 1:
        ...
        exerciseSearch = await HomeExerciseModel.findOne({
          _id: exercise._id,
        });
        ...
        if (imagesToSave.length) {
          result = await HomeExerciseModel.updateOne(
            { _id: exercise._id },
            {
        ...
        await HomeExerciseCategoryModel.populate(exerciseSearch, {
          path: "categoryId",
        });
        exerciseSearch.images = exerciseSearch.images.map(
          (image) =>
            `${req.protocol}://${req.headers.host}/images/home-exercises/${image}`
        );
    ...
    console.log(`Error in /home-exercises/edit, error: ${e.message}`, e);
    res.json({
      status: false,
      errors: [e.message],
    });
